...
public enum ApduResponseStatusWord {
    SW_UNKNOWN("6666"), // used to indicate that the Status word is unknown
    ...
    SW_6284("6284"), // Selected file is not valid. FCI not formatted according to ISO
    SW_6285("6285"), // Selected file is in termination state
    SW_62A2("62A2"), // Wrong R-MAC
    SW_62A4("62A4"), // Card locked (during clear( ))
    SW_62F1("62F1"), // Wrong C-MAC
    SW_62F3("62F3"), // Internal clear
    ...
    // we use the map for faster lookup
    private static Map<Byte, Entry> map = new HashMap<>();

    static {
        for (final ApduResponseStatusWord type : ApduResponseStatusWord.values()) {
            Entry e = map.get(type.statusWord[0]);
            if (e == null) {
                e = new Entry(type);
                map.put(type.statusWord[0], e);
            }

            if (type.statusWord.length == 1) {
                map.put(type.statusWord[0], e);
            } else {
                if (e.map.get(type.statusWord[1]) == null) {
                    e.map.put(type.statusWord[1], type);
                } else {
                    throw new IllegalStateException("Item already added: " + type);
                }
            }
        }
    }
    ...
    public final boolean isSecondByteVariable;


    ApduResponseStatusWord(final byte[] statusWord) {
        if (statusWord.length != 2 && statusWord.length != 4) {
            throw new IllegalArgumentException("Invalid length");
        }

        this.statusWord = statusWord;

        isSecondByteVariable = statusWord.length == 2;
    }


    ApduResponseStatusWord(final String statusWordString) {
        if (statusWordString.length() != 2 && statusWordString.length() != 4) {
            throw new IllegalArgumentException("Invalid length");
        }

        this.statusWord = ByteUtils.fromString(statusWordString);
        isSecondByteVariable = statusWordString.length() == 2;
    }


    public static ApduResponseStatusWord fromData(final byte[] data) {
        ApduResponseStatusWord ret = SW_UNKNOWN;

        if (data != null && data.length >= 2) {
            final Entry e = map.get(data[data.length - 2]);
            if (e != null) {
                final ApduResponseStatusWord twoByte = e.map.get(data[data.length - 1]);
                if (twoByte != null) {
                    return twoByte;
                } else {
                    if (e.sw.isSecondByteVariable) {
                        ret = e.sw;
                    }
                }
            }
    ...
    public static boolean isSuccess(final byte[] data) {
        if (data == null) {
            return false;
        }
        ...
        final ApduResponseStatusWord tmp = ApduResponseStatusWord.fromData(data);

        return tmp != null && tmp == SW_9000;
    }


    public static boolean areEqual(final byte[] data, final ApduResponseStatusWord data2) {
        if (data2 == null) {
            throw new NullPointerException("data2 is null");
        }

        final ApduResponseStatusWord tmp = ApduResponseStatusWord.fromData(data);

        return tmp != null && tmp == data2;
    }


    @Override
    public String toString() {
        return "StatusWord: " + ByteUtils.toHexString(statusWord);
    }


    private static class Entry {
        private ApduResponseStatusWord sw;
        private Map<Byte, ApduResponseStatusWord> map = new HashMap<>();


        private Entry(final ApduResponseStatusWord sw) {
            this.sw = sw;
        }
    }
